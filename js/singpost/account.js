	...
	$(document).on('click','a.buttone_save',function(){
		...
		if(rel == 'full_name'){
			...
			if(input == ''){
				...
				$(".row").eq(1).before($("<p class='accountError'>Please specify your full name.</p>"));
			...
			if(dd == ''){
				$(".row").eq(1).before($("<p class='accountError'>Please specify your date of birth.</p>"));
				$("#dd").addClass("errorBorder");
				return false;
			}
			else if(dd.length > 2 || dd.length < 2){
				$(".row").eq(1).before($("<p class='accountError'>Maximum of 2 characters for your date of birth.</p>"));
				$("#dd").addClass("errorBorder");
				return false;
			}
			else if($dd_num == 'true'){
				$(".row").eq(1).before($("<p class='accountError'>Please insert numbers only.</p>"));
				$("#dd").addClass("errorBorder");
				return false;
			}
			else if(mm == ''){
				$(".row").eq(1).before($("<p class='accountError'>Please specify your month of birth.</p>"));
				$("#mm").addClass("errorBorder");
				return false;
			}
			else if(mm.length > 2 || mm.length < 2){
				$(".row").eq(1).before($("<p class='accountError'>Maximum of 2 characters for month of birth.</p>"));
				$("#mm").addClass("errorBorder");
				return false;
			}
			else if($mm_num == 'true'){
				...
				$(".row").eq(1).before($("<p class='accountError'>Please specify year of birth.</p>"));
				...
				$(".row").eq(1).before($("<p class='accountError'>Please define your date of birth from 01  - 31 only.</p>"));
				$("#dd").addClass("errorBorder");
				return false;
			}
			else if(mm > 12 || mm < 01){
				$(".row").eq(1).before($("<p class='accountError'>Please define your month of birth from 01  - 12 only.</p>"));
				$("#mm").addClass("errorBorder");
				return false;
			}
			else if(yy > 2004 || yy < 1900){
				$(".row").eq(1).before($("<p class='accountError'>Please define your year of birth from 1900  - 2004 only.</p>"));
				$("#yy").addClass("errorBorder");
				return false;
			}
		...
		$.ajax({
		    ...
		    type: "post",
		    data:{value:input,attr:rel,oldVal:oldVal},
		    dataType: 'json',
		    success: function(data) {
				...
				if(data.response == 'error'){
					...
					{
						//alert(data.result[i]);
						$("#error_msg").show().text(data.result[i]);
						// setTimeout(function() {
							// $.fn.custombox('close');
						// }, 2000);
					}
				...
				else if(data.response == 'update')
				{
					if(data.result.submit == true)
					{
						...
						$("#error_msg").show().append("<p>"+data.result.message+"</p>");
						//close the form
						setTimeout(function() {
							//$.fn.custombox('close');
						}, 2000);
				...
				//window.location.href = "/profile/index/account/";
				//$(".right-content").load("profileAjaxLoad");
				//$(".profile-index-account").load("profileAjaxLoad");
		    }//end else
		});//end of ajax
	});//end of click
