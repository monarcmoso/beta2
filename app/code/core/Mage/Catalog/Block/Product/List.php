...
class Mage_Catalog_Block_Product_List extends Mage_Catalog_Block_Product_Abstract
{
    ...
    protected function _getProductCollectionEdit()
    {      
       if (!is_null($this->_productCollection)) {
            ...
            $collection = $this->_productCollection = $layer->getProductEditCollection($this->_productIds);
     ...
     * @return Mage_Eav_Model_Entity_Collection_Abstract
     */
    public function getLoadedProductCollectionEdit()
    {   
        return $this->_getProductCollectionEdit();
    }
    ...
    {   
        ...
        //$collection = $this->_getProductCollectionEdit();
    ...
    private $_itemPerPage = 12;
    private $_pageFrame = 100;
    private $_curPage = 1;
     //custom code by monarc
    ...
    {    
        ...
        if($collection->count() > $this->_itemPerPage || ($collection->count()>0 && $curPage!=1) )
        { 
            $curPage = $this->getRequest()->getParam('p');
            ...
            if ($count>1){
                for($i = $start; $i<=$count; $i++)
                {
                    if($i >= 1){
                        if($curPage){
                            $html .= ($curPage == $i) ? '<li class="current" style="font-weight: bold;">['. $i .']</li>' : '<li><a href="'.$url.'?p='.$i.'">'. $i .'</a></li>';
                        }else{ 
                        ...
                        if($count==$i){
                                ...
                                $p = $curPage+1;
                                $html .= ($i == 1 || $curPage==$count) ? '' : '<li><a href="'.$url.'?p='.$p.'">></a></li>';
                                $html .= ($i == $curPage) ? '' : '<li><a href="'.$url.'?p='.$i.'"> LAST </a></li>';
                        }
                    }
