...
class Mage_Checkout_Model_Type_Onepage
{
    ...
    public function saveBilling($data, $customerAddressId)
    {
        ...
        //billing/shipping information is saved for paypal purposes...
        $fullname = $data['firstname'];
        $shpping_postal = $data['postcode'];
        $addressModel  = Mage::getModel('customer/address');
        $addressId = Mage::getModel('profile/action')->_addressEntityId();
        $customer_id = Mage::getSingleton('customer/session')->getId();
        $addressModel->setCustomerId($customer_id)
        ->setFirstname($fullname)
        ->setStreet($shipping_address)
        ->setPostcode($shpping_postal)        
        ->setId($addressId);
       
        $saved = $addressModel->save();
        if($saved) {
              Mage::register('shipping_address', $shipping_address);
              Mage::register('postcode', $shpping_postal);
              Mage::register('fullname', $fullname);
        }
