...
class Codnitive_Sidenav_Block_Navigation extends Mage_Catalog_Block_Navigation {
    ...
    public function getCategoriesNavSideMenu() {
        $navigationMenu = $this->renderCategoriesMenuSideHtml(0);
        return $navigationMenu ? $navigationMenu : false;
    }
    ...
    $category, $level = 0, $isLast = false, $isFirst = false, $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false
    ) {
        ...
        if ($category->getId() != 10 && $category->getId() != 15 ) {
            ...
            $html[] = '<a '.$hidebottom_line.' '.$hide.' ' . $aClass . $onclick . 'href="' . $cat_link . '"'
                    . $linkClass . '>' .$checkbox.$label_start. ''
                    . $this->escapeHtml($category->getName()) .$label_end. '</a>';
          ...
          if($category->getId() == 18)  {  
           $htmlLi .= '<li >
                        ...
                        <li><a href="javascript:;"><input type="button" value="CLEAR" name="clear_all" id="clearme"></a></li>
                        <li><a href="javascript:;"><input type="submit" value="FILTER" name="filter"></a></li>
                        </ul>
                        </li>';
           $html[] = $htmlLi;
            }
    ...
    public function renderCategoriesMenuSideHtml($level = 0, $outermostItemClass = '', $childrenWrapClass = '') {
        $currentId = 18; //category parent menu
        $currentLevel = $this->_getCategoryModel()->load($currentId)->getLevel();
        if ($registerdCategory = Mage::registry('current_category')) {
            $currentId = 18;
            $registerdCategory->getId();
            $currentLevel = $registerdCategory->getLevel();
        }
    ...
    {
        $currentId =44;// Mage::app()->getStore()->getRootCategoryId();
        $currentLevel = $this->_getCategoryModel()->load($currentId)->getLevel();
        if ($registerdCategory = Mage::registry('current_category')) {
            $currentId = $registerdCategory->getId();
            $currentLevel = $registerdCategory->getLevel();
        ...
        foreach ($categories as $child) {
            ...
            $condition = ($child->getLevel() == $currentLevel)
                    && ($child->getId() != $currentId);
            if ($child->getIsActive() && !$condition) {
                $activeCategories[] = $child;
            }
