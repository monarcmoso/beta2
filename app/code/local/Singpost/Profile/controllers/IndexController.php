...
class Singpost_Profile_IndexController extends Mage_Core_Controller_Front_Action
{
	...
	public function editProfileAction()
	{
		...
		if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
		{
			...
			$attribute_code = $this->getRequest()->getPost('attr');
			$attribute_code = !empty($attribute_code) ? $attribute_code : array_push($error,'Update Failed. Please Contact the website Administrator.');
			$value = $this->getRequest()->getPost('value');
			$value = !empty($value) ? $value : array_push($error,'Please speciy your changes!');
			...
			if($attribute_code == 'dob')
			{
				...
				if($date[1] > 12 || $date[1] < 01){
					echo json_encode(array('response'=>'error','result'=>array('Invalid Date format')));
					exit;
				}
			...
			if($action == true)$message = 'Changes have been saved.';
	...
	public function fbProfileAction()
	{
		...
		$model =  Mage::getModel("profile/dashboard");
		$fbLoginCount = $model->countUserFbLogin($customer_id);
		// if($fbLoginCount == 1){
			$customer = Mage::getModel("customer/customer"); 
			$customer->load($customer_id);
