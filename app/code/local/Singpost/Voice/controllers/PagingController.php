...
class Singpost_Voice_PagingController extends Mage_Core_Controller_Front_Action
{
	public function _pageAjaxAction()
	{
		if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
		{
			...
			//print "<pre>";
			$arr = Mage::getModel("voice/action")->_getReviewsByPage($attributeValue,$limit,4);
			...
			$_reviewArray = array();
			$_reviewUsers = Mage::getModel("voice/action")->_reviews($attributeValue,$limit, 4);
			...
			$page_num = Mage::getModel("voice/action")->_getPageNum($attributeValue);
			//get questions here
			//$eff_questions = "";
			...
			echo $this->getLayout()->createBlock('lesson05/lesson05')->setData('limit', $limit)->setData('page_num', $page_num)->setData('attr', $attributeValue)->setData('efficacy',$eff_questions)->setData('data', $_reviewArray)->setTemplate('lesson05/reviews.phtml')->toHtml();
		}
		else
		{
