...
class Singpost_Login_AccountController extends Mage_Customer_AccountController 
{
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                ...
                try {
	            	...
	            	if(!$migrated > 0)
	            	{
						...
						if($facelift === true){
							try{
								$query = Mage::getModel("singpost_login/facelift");
								$customer->load($customerid);
					       		$customer->setPassword($login['password']);
					        	$customer->save();
					        	//update the table
					        	$session->loginById($customerid);
								$query->_updateStatus($login['username']);
								$query->_addPoints($login['username'],$customerid);
	...
	protected function _loginPostRedirect()
    {
		...
		if(Mage::getSingleton('customer/session')->isLoggedIn()){
			...
			if($data[0]['count'] == 0)
			{
				$this->_redirectUrl('/profile/index');
			}
			else
			{
