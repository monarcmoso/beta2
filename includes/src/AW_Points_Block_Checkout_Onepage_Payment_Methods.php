...
class AW_Points_Block_Checkout_Onepage_Payment_Methods extends Mage_Checkout_Block_Onepage_Payment_Methods
{
    ...
    public function getRequiredPoints($points = null){
        
        if(!$points)  return false;
        
         $free_count =  Mage::registry('free_count');
         $points_to_deduct = 0;
         (int)$points_to_deduct = $free_count * Mage::helper('points/config')->getPointsPerItem();
         
         if($points_to_deduct<=$points)  return true;
       
        return false;
    }
    public function getPointsToDeduct(){
    
         $free_count =  Mage::registry('free_count');
        return $points_to_deduct = $free_count * AW_Points_Helper_Config::ITEM_RULES;
    ...
    {  
        $isAvailable = $this->getSummaryForCustomer()->getPoints()
            ...
            && $this->getRequiredPoints($this->getSummaryForCustomer()->getPoints())
        ;
