...
class Mage_Checkout_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    //this is the model for the hijack of the points and rewards!
    public function setPoints($customer_id, $newpoints , $order_id, $points_summary_id, $points_deduct )
    {
           $db = Mage::getSingleton('core/resource')->getConnection('core_read');
	    $sql = "SELECT * FROM aw_points_transaction where summary_id=$points_summary_id and order_id=$order_id";
	   $rows = $db->fetchAll($sql);
            //echo $lastInsertId;
	if(empty($rows)){
            $customerData = Mage::getModel('customer/customer')->load($customer_id)->getData();
            
            $comment = 'Deduct Points for Checkout';
            ...
            $deductsSql = "INSERT INTO aw_points_transaction (store_id, summary_id, order_id, balance_change, balance_change_spent, action, comment, notice, change_date, customer_name, customer_email, is_locked)
             VALUES (1, $points_summary_id, $order_id, -$points_deduct, 0, 'order_invoiced', '$comment', '', NOW(), '$name', '$email', 0)";
