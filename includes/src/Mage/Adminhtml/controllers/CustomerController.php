...
class Mage_Adminhtml_CustomerController extends Mage_Adminhtml_Controller_Action
{
	...
	public function disableUserAction()
	{
		if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
		{
			...
			$isActive = Mage::getModel('customer/customer')->load($customer_id);
			$analytics_logs = Mage::getModel("profile/analytics");
			
			if($isActive->getIsActive() == 1){
				try {
					$writeConnection = Mage::getSingleton('core/resource')->getConnection('core_write');
					$query = "UPDATE customer_entity SET is_active = '0' WHERE entity_id = ".(int) $customer_id;
					$writeConnection->query($query);
					Mage::getSingleton('adminhtml/session')->addSuccess('You have successfully disable this user account.');
					$analytics_logs->addAnalyticsLogs('deleted',$customer_id);
				...
				Mage::getSingleton('adminhtml/session')->addError('Account is already deactivated.');
				echo json_encode(false);
			}
		}
		else{
