   ...
   public function addAjaxAction()
    {   header('Content-type: application/json');
        ...
        foreach ($_wishlistitemCollection as $_wishlistitem) {           
            $_itemsInWishList = $_wishlistitem->getProduct();
           if($_wishlistitem->getProductId() ==$productId) {
                  $item_exist = true;
                  $response_array['status'] = 'have'; 
              
           }    
        }
        if($item_exist){
             $result = $this->_removeItemToWishListAjax();
           
        }
        else 
            $result = $this->_addItemToWishListAjax();
        if($result && !$item_exist){
              $response_array['status'] = 'success';  
        }
        else if(!$result && !$item_exist )
        {
               $response_array['status'] = 'error';  
        }
    ...
    {    
        ...
        try {
            ...
            $message = $this->__('%1$s has been added to your interest. Click <a href="%2$s">here</a> to continue shopping.',
        ...
        }  //return to previous product detail page and increment the interest count
    ...
    public function removeAction()
    {
     ...
     public function _removeItemToWishListAjax(){
         $session = Mage::getSingleton('customer/session');
         $product_id = (int) $this->getRequest()->getParam('product');
         $customerId = Mage::getSingleton('customer/session')->getCustomerId();
         $wishlist = Mage::getModel('wishlist/wishlist');
          
         $wishlist_data =  $wishlist->loadByCustomer($customerId, true);
         $wishlist_id = $wishlist_data->getId();
        ...
        $wishlist_sql = "SELECT * FROM wishlist_item where product_id=".$product_id." and wishlist_id=".$wishlist_id."";
        $arr = $read->fetchAll($wishlist_sql);
        $id = 0;
        foreach($arr as $key ){
           $id = $key['wishlist_item_id'];
      ...
      /***
     * ajax call to request  product to be added in the wishlist 
     * ***/
    protected function _addItemToWishListAjax()
    {
        $wishlist = $this->_getWishlist();
        if (!$wishlist) {
    ...
    public function fromcartAction()
    {
        ...
        try {
            ...
            Mage::getSingleton('core/session')->addSuccess(Mage::helper('wishlist')->__("%s has been moved to wishlist %s", $productName, $wishlistName));
            $wishlist->save();
        } catch (Mage_Core_Exception $e) {
            $session->addError($e->getMessage());
        } catch (Exception $e) {
