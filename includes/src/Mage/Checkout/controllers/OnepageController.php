...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function _model() //get model of otp
    {
    ...
    {       
        ...
        $addresses = array();
        $i=0; foreach ($customer->getAddresses() as $address):
        $addresses = $address->toArray();
        $postal_code =  $addresses['postcode']; 
        $shipping_address = $addresses['street']; 
        $i++; endforeach;  
        
        
        if(!$postal_code){
            Mage::getSingleton('core/session')->addNotice('Please complete your billing address before you make a checkout. Thanks');
            $this->_redirect('profile/index/settings/src/shipping');
        }
	...
	$rows = $db->fetchAll($sql);
        ...
        foreach($rows as $row){
          ...
          if($launch || $expired || !$active){
                 Mage::register('show_coupon', 1);
            }else{
                Mage::register('show_coupon', 0);
            } 
            
            
            
            //get number of paid and number of free samples in the cart
            ...
            foreach($quote->getAllItems() as $item)
            {
                ...
                if($free_delivery_bool != 'Yes') ++$free_count;
                else ++$paid_count;
            }
            ...
            //echo $mobile_verified = $this->_model()->_getUserIfverified();
           
            Mage::register('mobile_verified', json_encode(array('result'=>$mobile_verified)));
              Mage::getModel('core/session')->setData('mobile_verified',$mobile_verified);
            Mage::register('paid_count', $paid_count);
            
            Mage::getModel('core/session')->setData('sreePoints',$free_count);
        ...
        $this->getLayout()->getBlock('head')->setTitle($this->__('Checking out your order - Sample Store'));
        $this->renderLayout();
    }
    ...
    {
            //deduct points -monarc
	  $ctr = 0;
	  $customer_id = Mage::getSingleton('customer/session')->getId();
          $session= Mage::getSingleton('checkout/session');  
	  $lastOrderId =  Mage::getSingleton('checkout/session')->getLastOrderId();
	  $order_items= Mage::getModel('sales/order')->load($lastOrderId);
          foreach($order_items->getAllItems() as $item) {
	     ...
	     $points_summary_id = $summaryForCustomer->getId();
             ...
             if($user_points>=$points_amount && $points_amount>0){
             
                $new_points = $user_points -$points_amount;
           
                $lastOrderId =  Mage::getSingleton('checkout/session')->getLastOrderId();
		
                Mage::helper('checkout')->setPoints($customer_id, $new_points , $lastOrderId ,$points_summary_id,$points_amount);
               
             }
             }catch (Mage_Payment_Model_Info_Exception $e) {
		   $message = "Your point(s) is not enough to do a checkout.";	
		  $this->_getSession()->addError($message);
	    }
		
            $customer_id = Mage::getSingleton('customer/session')->getId(); 
            $orders = Mage::getModel('sales/order')->getCollection()
            ->addFieldToFilter('status', array('in' => array('processing','packing')))
            ->addFieldToFilter('customer_id', $customer_id)            
            ; $i=0;
            foreach($orders as $order){
                if($order->getStatus()=='processing' || $order->getStatus()=='packing' || $order->getStatus()=='sent') $i++;
            }

            if($i>1)
             {
                Mage::getSingleton('core/session')->addSuccess('Thank you, your payment was submitted successfully. An order confirmation email will be sent to you shortly. '); 
        ...
        $this->getLayout()->getBlock('head')->setTitle($this->__('Thank you for your order - Sample Store'));
        $this->renderLayout();
    }
    ...
    public function billingInfoAction(){
        ...
        $customer_data = $customerId->getData();
        ...
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
		{
			...
			try{    
				; //$this->_model()->_getUserIfverified();
				
                                echo json_encode(array('fullname'=>ucfirst($fullname),'street'=>$shipping_address,'postal_code'=>$postal_code,'mobile_no'=>$mobile_no));
                                exit;
			} catch(Exception $e) {
    ...
    public function saveOrderAction()
    {
        ...
        try {
            ...
            $this->getOnepage()->getCheckout()->getRedirectUrl();
